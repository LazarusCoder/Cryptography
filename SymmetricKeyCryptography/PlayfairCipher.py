...
"""
import numpy as np
plaintext=list("talltrees".replace("j","i"))
key=list('occurrence'.replace("j","i"))
encrypt=""
matrix=[]
for i in key:
...
while k+1<len(plaintext):
  if(plaintext[k]==plaintext[k+1]): #index for x
    index.append(k+1)
  k+=2
print(plaintext)
...
for i in range(len(index)):
  ...
  for j in range(0,len(plaintext),2):
    if(j+1<len(plaintext)):
      if(plaintext[j]==plaintext[j+1]):     #repeatedly check for same letter
        status=0
